...
class BeamSearchDecoder(object):
  ...
  def decode(self):
    ...
    while True:
      ...
      rst = batch.rst_batch[0]

      original_request = batch.original_requestss[0]  # string
      original_article = batch.original_articles[0]  # string
      original_abstract = batch.original_abstracts[0]  # string
      original_abstract_sents = batch.original_abstracts_sents[0]  # list of strings

      request_withunks = data.show_art_oovs(original_request, self._vocab) # string
      article_withunks = data.show_art_oovs(original_article, self._vocab) # string
      abstract_withunks = data.show_abs_oovs(original_abstract, self._vocab, (batch.art_oovs[0] if FLAGS.pointer_gen else None)) # string

      # Run beam search to get best Hypothesis
      pred, best_hyp = beam_search.run_beam_search(self._sess, self._model, self._vocab, batch)
      ...
      if FLAGS.single_pass:
        self.write_for_rouge(original_abstract_sents, decoded_words, counter, int(rst[0]), int(pred)) # write ref summary and decoded summary to file, to eval with pyrouge later
        y_true.append(int(rst[0]))
        y_pred.append(int(pred))
        # print(int(rst[0]))
        # print(int(pred))
        counter += 1 # this is how many examples we've decoded
      else:
        print_results(request_withunks, article_withunks, abstract_withunks, decoded_output) # log output to screen
  ...
  def write_for_rouge(self, reference_sents, decoded_words, ex_index, rst=0, pred=0):
...
def print_results(request, article, abstract, decoded_output):
  """Prints the article, the reference summmary and the decoded summary to screen"""
  print("---------------------------------------------------------------------------")
  tf.logging.info('CLAIMS:  %s', request)
  tf.logging.info('FACTS:  %s', article)
  tf.logging.info('REFERENCE COURTVIEW: %s', abstract)
  tf.logging.info('GENERATED COURTVIEW: %s', decoded_output)
  print("---------------------------------------------------------------------------")
