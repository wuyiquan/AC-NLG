...
# Where to find data
tf.app.flags.DEFINE_string('data_path', '../data/finished_files/chunked/val_*',
                           'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string('vocab_path', '../data/finished_files/vocab', 'Path expression to text vocabulary file.')
...
tf.app.flags.DEFINE_string('exp_name', 'TEST',
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer):
    ...
    with sess_context_manager as sess:
        ...
        while True:  # repeats until interrupted
            ...
            loss_2 = results['loss_2']
            tf.logging.info('loss_2: %f', loss_2)  # print the loss to screen

            loss_pred = results['loss_pred']
            tf.logging.info('loss_pred: %f', loss_pred)  # print the loss to screen

            if not np.isfinite(loss_1) and not np.isfinite(loss_2):
            ...
            if FLAGS.coverage:
                ...
                coverage_loss_2 = results['coverage_loss_2']
                tf.logging.info("coverage_loss_2: %f", coverage_loss_2)  # print the coverage loss to screen
...
def run_eval(model, batcher, vocab):
    ...
    running_avg_loss_2 = 0  # the eval job keeps a smoother, running average loss to tell it when to implement early stopping
    running_avg_loss_pred = 0
    ...
    while True:
        ...
        if FLAGS.coverage:
            ...
            coverage_loss_2 = results['coverage_loss_2']
            tf.logging.info("coverage_loss_2: %f", coverage_loss_2)
        ...
        running_avg_loss_2 = calc_running_avg_loss(np.asscalar(loss_2), running_avg_loss_2, summary_writer, train_step)
        running_avg_loss_pred = calc_running_avg_loss(np.asscalar(loss_pred), running_avg_loss_pred, summary_writer, train_step)


        # If running_avg_loss is best so far, save this checkpoint (early stopping).
        # These checkpoints will appear as bestmodel-<iteration_number> in the eval dir
        if best_loss is None or running_avg_loss_1+running_avg_loss_2+running_avg_loss_pred < best_loss:
            tf.logging.info('Found new best model with %.3f running_avg_loss. Saving to %s', running_avg_loss_1+running_avg_loss_2+running_avg_loss_pred,
                            bestmodel_save_path)
            saver.save(sess, bestmodel_save_path, global_step=train_step, latest_filename='checkpoint_best')
            best_loss = running_avg_loss_1+running_avg_loss_2+running_avg_loss_pred
