...
def run_beam_search(sess, model, vocab, batch):
  ...
  hyps = [Hypothesis(tokens=[vocab.word2id(data.START_DECODING)],
                     ...
                     attn_dists=[],
                     p_gens=[],
                     coverage=np.zeros([batch.art_batch.shape[1]]) # zero vector of length attention_length
  ...
  while steps < FLAGS.max_dec_steps and len(results) < FLAGS.beam_size:
    ...
    (pred, topk_ids, topk_log_probs, new_states, attn_dists, p_gens, new_coverage) = model.decode_onestep(sess=sess,
                        batch=batch,
